#!/usr/bin/env python3
"""Render the 1200x630 social card for llms.2plot.dev (a 2plot satellite).
    ...
    python scripts/make_social_card.py \
        --artwork assets/favicon/android-chrome-512x512.png \
        --brand "dash-improve-my-llms" --domain llms.2plot.dev
...
def load_font(kind: str, size: int):
    ...
    print(
        f"[card] WARNING: no {kind} system font found — falling back to "
...
def main() -> int:
    ...
    ap.add_argument(
        "--artwork",
        default="assets/favicon/android-chrome-512x512.png",
        help="source image, transparent PNG (default: %(default)s)",
    )
    ap.add_argument(
        "--brand",
        default=SITE_BRAND.split(" — ")[0],
        help="headline (default: the brand, minus its tagline)",
    )
    ap.add_argument(
        "--tagline",
        default="Make a Plotly Dash app readable to search engines, " "crawlers and AI agents.",
    )
    ap.add_argument("--domain", default=default_domain)
    ap.add_argument("--out", default=None, help="default: build/social-cards/<domain>.png")
    ap.add_argument("--open", action="store_true", help="preview when done (macOS)")
    args = ap.parse_args()

    artwork = (
        (REPO_ROOT / args.artwork) if not Path(args.artwork).is_absolute() else Path(args.artwork)
    )
    if not artwork.exists():
        return print(f"artwork not found: {artwork}", file=sys.stderr) or 1

    out = (
        Path(args.out) if args.out else REPO_ROOT / "build" / "social-cards" / f"{args.domain}.png"
    )
    ...
    print(
        "[card] then update OG_IMAGE_URL / OG_IMAGE_WIDTH / OG_IMAGE_HEIGHT " "in lib/constants.py"
    )
