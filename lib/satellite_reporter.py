...
def app_key() -> str:
    ...
    Default is "llms" — THIS host's directory key (deviation from the
    boilerplate source, whose fallback is its own "boilerplate" key: a
    deployment that lost the env var must degrade to reporting under its
    own identity, never overwrite another satellite's rows at the hub).
    render.yaml sets SATELLITE_APP_KEY=llms explicitly regardless.
    """
    return os.getenv("SATELLITE_APP_KEY") or "llms"
