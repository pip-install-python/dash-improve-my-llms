...
def register_quart(app: Any, config: Any, state: Any) -> None:
    ...
    async def _wk_response(payload):
    ...
    async def _wk_api_catalog():
        return await _wk_response(
            (
    ...
    async def _wk_mcp_card():
        card = wellknown.build_mcp_server_card(app, config)
        if card is None:
            return await _wk_response((wellknown.not_found_body(), wellknown.JSON_TYPE, 404))
        return await _wk_response((card, wellknown.JSON_TYPE, 200))

    async def _wk_agent_skills():
        return await _wk_response(
            (wellknown.build_agent_skills_index(app), wellknown.JSON_TYPE, 200)
        )

    async def _wk_guard(rest=""):
        return await _wk_response((wellknown.not_found_body(), wellknown.JSON_TYPE, 404))
