...
def _visit(path, *, minute=0, ip="1.1.1.1", ua="Mozilla/5.0 Chrome", device_type="desktop"):
    return {
...
def test_a_machine_surface_only_day_is_reported(tmp_path):
    ...
    ledger = _ledger(
        ...
        [
            ...
            _visit("/llms.txt", minute=3, ip="3.3.3.3", ua="ClaudeBot", device_type="bot"),
        ],
    )
    payload = daily_rollup(
        "boilerplate", DAY, visits=load_visits(ledger), agent_visits=load_agent_hits(ledger)
    )
    ...
    assert rows["/llms-full.txt"] == {"path": "/llms-full.txt", "hits": 2, "bot_hits": 2}
