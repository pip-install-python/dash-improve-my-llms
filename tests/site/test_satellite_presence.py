...
def _visit(path, *, dt, ip="1.1.1.1", ua="Mozilla/5.0 Chrome", device_type="desktop"):
    return {
...
def test_active_counts_distinct_humans_inside_the_session_window(ledger, monkeypatch):
    now = datetime.now()
    ...
    ledger(
        [
            ...
            _visit(
                "/e", dt=fresh, ip="4.4.4.4", ua="GPTBot/1.0", device_type="bot"
            ),  # bots never count
        ]
    )
...
def test_presence_url_derives_from_the_traffic_override(monkeypatch):
    ...
    monkeypatch.setenv("SATELLITE_TRAFFIC_URL", "https://staging.example/api/satellite/traffic")
...
def test_a_failed_post_is_swallowed_never_raised(monkeypatch):
    ...
    ok, detail = sr._post_signed(
        "https://2plot.ai/api/satellite/active",
        {"app": "t", "active": 1},
        "presence-beacon",
        secret="s",
    )
...
def test_the_presence_post_sends_the_internal_token(monkeypatch):
    ...
    ok, _ = sr._post_signed(
        sr.presence_endpoint(), {"app": "t", "active": 0}, "presence-beacon", secret="test-secret"
    )
    assert ok is False
